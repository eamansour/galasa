...
- **Personal Access Token Expiry**: Personal access tokens now have expiry times associated with them. When a token expires, it can no longer be used to authenticate with the Galasa service.
  - All existing tokens will migrate to expire in 90 days from the date of the update, however, they might expire sooner due to dex (see your helm chart).
  ...
  - Tokens are automatically disabled when they reach their expiry date. Note: if you create a JWT close to the expiry date, then you might notice that your token will continue to work up to 24 hours after the expiry date, this is because the token is still valid in the JWT library.
  - The Galasa CLI will automatically delete your token if you attempt to use it after it has expired.
  - The CLI displays a warning (GAL2001W) when using a token that is approaching its expiry date.
